...
适用范围：苏轼诗文 N 选 64 项目首页（入口页），包含标题区、作者说明框、两张入口卡片（N选64 / 排行榜）、随机诗文句四部分。整体风格为宋代雅趣的中式素雅配色（米白+墨绿），衬线宋体字体，严格响应式布局。
...
#app-root  根容器（全局约束宽度 + 居中）
 └─ .homepage  首页主体（fadeIn 进场动画）
     ...
     │   │   ├─ .title-line  左侧装饰线
     │   │   ├─ h1.home-title  主标题「苏轼诗文N选64」
     │   │   └─ .title-line  右侧装饰线
     │   └─ p.home-subtitle  副标题「— 填表游戏 —」
     │
     ├─ .author-note-wrapper  作者说明条
     │   ├─ .author-note-bar  左侧墨绿色条（宽 4px）
     │   └─ .author-note-box  说明正文框（虚线边框）
     │
     ├─ .entry-cards  入口卡片组（flex 居中、换行，卡片并列两张）
     │   ├─ .entry-card  卡片 1：苏轼诗文 / N选64（跳转 #/nxuan64，mode='64'）
     │   │   ├─ .card-deco-bar
     │   │   └─ .card-body → .card-title / .card-subtitle
     │   └─ .entry-card  卡片 2：排行榜 / 截至26.8.29（跳转 #/ranking，mode='rank'）
     │       ├─ .card-deco-bar
     │       └─ .card-body → .card-title / .card-subtitle
     │
     └─ .random-poem  底部随机诗文句
         ├─ 「内容句」
         └─ .random-poem-title  出处标题
```
...
#### 卡片排列与 hash 路由映射（两张卡片）

| 序号 | 卡片标题 / 副标 | 点击行为（switchMode） | hash 值 | 目标页面 mode |
|---|---|---|---|---|
| 左 | 「苏轼诗文」/「N选64」 | `switchMode('64')` | `#/nxuan64` | `mode='64'` → N选64 填表页 |
| 右 | 「排行榜」/「截至26.8.29」 | `switchMode('rank')` | `#/ranking` | `mode='rank'` → 排行榜页 |

主页（mode=null，hash 为空或 `#/`）时展示以上两张卡片；点击跳转通过 `window.location.hash` 修改，`hashchange` 事件同步 `this.mode`。

#### `.entry-card` 单张卡片（两张样式完全一致）
